...
type Option struct {
	...
	// With Terminator ; (semi-colon), "--opt" will receive
	// {"a", "--b=c", "--", "-d", "x y"} as it's argument.
...
func (option *Option) SetTerminatedOption(value []string) error {
	...
	if tp != reflect.TypeOf((*[][]string)(nil)).Elem() {
		return fmt.Errorf("Terminated options do not have support for %v fields yet", tp)
	}
	...
	option.value.Set(reflect.Append(option.value, reflect.ValueOf(value)))
	return nil
}
